...
/**
 * Creates the account, with no domain yet. This runs on a first successful
 * sign-in, whichever provider carried it.
 *
...
export interface AccountStore {
   ...
   * `created` distinguishes a signup from a returning sign-in, and the
   * insert-with-conflict is what stops two simultaneous callbacks both
   * creating an account.
   */
...
/** One step of the activation funnel: `preview_requested → preview_served → signup_completed → …`. */
export const SIGNUP_COMPLETED_EVENT = 'signup_completed'

/**
 * Email is the account identity, so a Google sign-in and an email sign-in on
 * the same address are the same account rather than two.
 *
 * `signup_completed` fires only on the create. Capturing it on every sign-in
 * would inflate the funnel and, with it, our cost per acquired signup. The
 * event carries no domain group: that group is reserved for claimed domains,
