...
- `title_tag` and `meta_description` — the current value, your suggestion, and
  a `rationale`. Write the suggestion in the language of the page. Leave
  `suggested` equal to `current` only if you genuinely cannot improve it.
- `rationale` — one plain sentence to the shop owner saying why your suggestion
  is better than what is there now, said only from the evidence you were given.
  They read it exactly as you write it, so write a sentence: not a label, not a
  code, not a category name. Write it in English whatever language the page is
  in, because it is read beside our own English wording rather than on the
  page. Use `null` when you have nothing to add beyond the suggestion itself;
  do not pad it.
...
- **Say something the store itself knows.** Where the store has recorded a fact
  that bears on a section or an answer you are proposing, cite at least one of
  those store facts in its `facts_used`, alongside whatever you take from the
  pages ranking above it. A passage built only out of what rivals cover tells
  the shop owner what everyone else says; the reason to buy from *them* is in
  their own facts. Where the store has recorded nothing on that subject,
  propose it anyway from the ranking pages and cite those — a subject the store
  has never written about is often exactly the gap worth telling them about.
