...
- **Sync:** in the daily reconciliation sweep (§14.1) — same budget, same checksum diffing — and on `collections/update` / `articles/*` webhooks. Changing pages update `store_pages.checksum`, which invalidates cached intent-gap analyses (§10.3) and re-scores open opportunities on that URL at the next scan.
...
- Subscribe to Shopify webhooks: `products/update`, `products/delete`, `collections/update`, `inventory_levels/update`, `app/uninstalled`, plus the mandatory GDPR topics (§14.6). Handler: verify HMAC signature, dedupe on `X-Shopify-Webhook-Id`, enqueue, return `200` in <5s (slow/failed responses cause Shopify to retry and eventually drop the subscription). All processing is async.
