...
- **T7.1b — patterns. DONE**, merged 2026-09-09. **It was narrower than this line said:** the multiplier is already *consumed* by `packages/core/src/opportunities/scoring.ts` and by the calendar top-up in `packages/core/src/calendar/replenishment.ts`, both of which behave correctly against the empty table they find today. **It is the writer that is missing.** Start from `ratedArticleLabels`, which `T7.1a` added for this. Two findings came out of it and are carded below: `T7.1-FAMILY-NAME` and `T7.1-TWO-THIRDS`.

**T7.1-FAMILY-NAME — the database and everything else disagree about the name of one axis** · **needs a schema wave**, then Lane C · found 2026-09-09 by `T7.1b`
Scope: the `pattern_dimension` enum spells the product-family axis `family`; the rules config (`signals.config.yaml`) and the already-shipped reader both spell it `family_id`. Invisible while the table was empty, and it would have begun mattering the moment anything wrote a row — **a family pattern stored under a name nobody asks for influences nothing, silently, and looks perfectly fine in the table.** `T7.1b` reconciled the two spellings inside the single file that both writes and reads that column, which is what makes it work rather than silently not work; migrations land only in schema waves, so the enum could not be changed.
Done when: the enum member is renamed and the translation in the writer is deleted, so there is one spelling rather than two and a bridge.

**T7.1-TWO-THIRDS — the threshold excludes the exact case it was written to include** · **a decision, not a defect to fix quietly** · `packages/rules` is Lane C's
Scope: the spec tilts a pattern at "two-thirds or more winners". The shipped number is `dominance_share_min: 0.6667`, which sits *above* two-thirds (0.6666…). So a group that is **exactly** two-thirds winners reads as mixed at any size — and at the minimum group size of three articles, that means a group must be **unanimous** to earn a tilt: three of three, never two of three.
Why it was left: the number is in another lane's file, the reader that already shipped compares the same way against the same value, and changing one side alone would make the two disagree. **The fix is one digit** and it changes how readily the product learns from itself, which is why it is a decision rather than a tidy-up.
Done when: the founder has said whether "two-thirds or more" means what it says, and both sides move together.

**T7.1-JOB — the weekly label run has no job behind it** · **DONE**, merged with `T7.1b` · Lane D, small
...
**LANDED 2026-09-07, with a named residue the card deliberately did not fix.** File-level failures, connection refusals and silently skipped suites are gone. What remains: a handful of tests that measure elapsed wall-clock time and exceed their budget when thirty workers share twelve cores. Four were named — `generation/replenish`, `providers/shopify/limiter`, `runtime/runtime`, `generation/veto-topic` — and a fifth sighting has since been reported: **`packages/jobs/src/runtime/runtime.test.ts:571`, "frees the store even when the unlock itself fails"**, failing once under full concurrent load and then passing 60/60 five runs in isolation. **A sixth member, and the only one with two independent sightings:** `apps/web/app/(public)/_lib/signin-wire.test.ts`, "hands the browser to Google, as this deployment's own client" — seen first on 2026-09-07 by the integrator and again the same evening by `sortiva-a8`, which caught it at 5004 ms (a timeout, not an assertion) with the load average at 31.24, the highest recorded that day, and then passed it 4 of 4 in isolation three times over. The second sighting was in a session whose diff did not touch that file. Fixing these means changing an assertion or raising a timeout, which is why they were left. **Treat a failure in one of these six under load as unproven rather than as a flake, and capture the name before re-running.**
...
### From `R-OPTIMIZE-AXES`, landed 2026-09-08

**R-GRADER-SEES-MORE — the quality grader is shown evidence the writer was forbidden** · Lane E · **the sharpest of the three, and it is live for prices**
Scope: a recommendation may only suggest copy that points at a named fact about a product. Three fields are deliberately withheld from the writer — the price range, a flag about the distillation, and the option-axis names. **The grader is handed the whole fact sheet** (`packages/core/src/optimize/grading.ts:90` sends `product.factSheet`; the writer gets only the citable fields, filtered at `pack.ts:183`).
**So the grader can mark a claim well-grounded on evidence the writer never saw.** Dormant for the axis names — the writer cannot see them at all, which a test now pins — but **live today for price**: the grader sees a price range the writer was never shown, so a sentence the model invented about price can be confirmed by a fact that was never in front of it.
**Why this is not the same as the judge being blind to the writer's context** (invariant 11), and the difference matters: that rule keeps the judge independent by giving it *less*. This gives it *more*, which is the opposite failure — independence is preserved and grounding is not.
...
**Read the fix next door before writing one.** `R-GRADER-SEES-MORE` (landed 2026-09-08) made writer and grader read one function, so a field that is not citable is invisible to both together, and its test works on **values rather than field names** — every primitive the judge is sent must appear in what the writer was sent, so a leak through a new key or a nested object fails too, and a new non-citable field will not compile until someone gives it a sentinel. **Follow that, do not invent a second scheme.**
**Do not close it the wrong way.** The judge is blind to the writer's *conversation* — that is what keeps it independent, and it is about withholding the reasoning, not the evidence. The neighbouring card checked this against the spec rather than from memory and wrote a test asserting the judge still cannot see the page body, the search numbers or the store's voice. Yours should too.
Read first: `DECISIONS.md` 2026-09-08 `R-GRADER-SEES-MORE`; main §8.4; invariant 11.
Done when: the article judge grades grounding against the same evidence the writer was given; a test fails if the two drift apart again; and the judge's independence is separately pinned rather than assumed.

**R-AXES-DESCRIPTION — a field's description says something the code does not store** · Lane B, small
Scope: `packages/core/src/distill/schema.ts:24-27` describes the field as holding "the axes the product's own variants differ along". It does not: an option with a single value, or none at all, still yields an axis name, because Shopify hands one back whether the merchant filled it or not. **That gap between the description and the content is the entire hinge of `R-OPTIMIZE-AXES`'s judgement** — a shop with one black shoe and a shop with five colourways store the identical word.
Read first: `DECISIONS.md` 2026-09-08 `R-OPTIMIZE-AXES` and 2026-09-07 `R-PRODUCT-ATTRIBUTES`; main §6.3.
Done when: the description says what is stored, so the next reader does not build on the sentence rather than the content.
Note: the description is also read by a model. **Changing it changes a prompt**, so treat it as a prompt edit and say what you changed it to.

**R-EVIDENCE-BLACKOUT — how long may a card sit on a merchant's screen while we cannot see its evidence?** · **ANSWERED 2026-09-08; COPY DONE, merged as `7bf21ec`; BEHAVIOUR NOT BUILT**
Founder's answer: hold the cards **indefinitely**, recorded as a deliberate hold, **and the screen says the evidence is stale because nothing is connected.** The two approved sentences are in the catalogue (`badge.evidenceStale`, `badge.evidenceStale.body`).
**What is missing, and it is the larger half: nothing marks a suggestion as held for this reason**, so those sentences have no producer and render nowhere. `limitedIntelligence` is a *different* state — a store that never connected Search Console at all, not one whose connection went away after the evidence was gathered. Carded below as `R-EVIDENCE-BLACKOUT-STATE`.

**R-EVIDENCE-BLACKOUT-STATE — a suggestion is marked as held when the connection behind its evidence is gone** · **Lane C** · the behaviour half of the founder's 2026-09-08 answer
Scope: an opportunity whose evidence came from Search Console, on an account whose Search Console connection has since gone, is held rather than expired — and carries something the screen can read to show the two approved sentences. The hold is recorded as deliberate, not as a failure, and **never expires**: that was the decision, so nothing may quietly time it out.
Read first: `DECISIONS.md` 2026-09-08 `R-EVIDENCE-BLACKOUT`; the two sentences in `packages/ui/strings/en.json`; how `limitedIntelligence` is set and why it is not this; main §7.9, §7.11; invariant 22.
Done when: an account that loses its Search Console connection has its affected cards marked held rather than expired; the screen shows the approved sentences; reconnecting clears the hold; and a test proves a held card is still there after the period an expiry would have removed it.
Scope: `R-EXPIRY-GAPS` (landed 2026-09-08) fixed the dangerous half of this — a store whose search data stops arriving used to read exactly like a store where every page had suddenly come right, so the weekly pass retired that merchant's whole board at once. **Those cards are now held open, and the pass logs how many it held.** That is deliberate and it is the safe direction.
**What it leaves:** they are held *indefinitely*. The card asked for "a stated fate rather than an accidental immortality"; the fate is now stated and it is still that they stay. The lane declined to invent an ageing-out rule for two good reasons: the spec promises that reconnecting Search Console **re-scores the existing cards**, which needs them to still exist; and "retire a card whose evidence we have not seen for N weeks" needs an **N**, which is a threshold, and thresholds are a product decision rather than something an expiry fix may pick.
**The question:** should a card whose evidence we have been unable to see for some weeks retire anyway, and after how long? Against retiring: a merchant who reconnects gets their board back intact. For retiring: a store that never reconnects accumulates advice nobody can act on or verify.
...
Done when: the founder has said, and a renamed post either carries the merchant's answer with it or deliberately does not.

**R-UNRECORDED-KEY — the new "no reason recorded" key is invisible to the guard that watches for keys with no words** · Lane F, small
Scope: `R-WRONG-GATE-REASON` (landed 2026-09-08) stopped a held calendar day borrowing another gate's sentence, and introduced `gate.reason_unrecorded` for a decision row that recorded no reason at all. **A merchant reads the renderer's honest fallback today — "The reasoning for this one isn't available yet." — so nothing is wrong on screen.** But the key appears in exactly two places, the route that emits it and its own tests: it is **not** in the copy catalogue and **not** in `packages/ui/src/strings/reason-copy.ts`'s list of keys the product produces.
**That guard exists precisely to fail when a produced key has no sentence.** A key it does not know about is the same fault in a new place — the eleventh instance of the day's pattern, arriving inside a card that was fixing an instance of it.
**The decision inside this card, and it is small:** either give the key purpose-written words, or register it as a declared known gap so the guard counts it. The second is defensible — an admission that we did not record a reason may be exactly the right sentence — but it should be a choice on the record rather than an absence.
Read first: `DECISIONS.md` 2026-09-08 `R-WRONG-GATE-REASON`; `packages/ui/src/strings/reason-copy.ts`.
Done when: the guard knows about the key, and the key either has a sentence or is listed as a deliberate gap.
...
Scope: two code paths take the same three row locks in opposite orders, so when a merchant presses "not interested" at the moment the publish hour hands their article over, **Postgres kills one of them with "deadlock detected"**.
- Handing an article over (`markArticleDelivered`, `packages/db/src/repositories/delivery.ts:50`) locks **the article first**, then the topic, then the opportunity.
- Calling the day off (`cancelDay`, `packages/jobs/src/generation/veto-topic.ts:112`) locks **the topic first**, then the article, then the opportunity.
That is a cycle on article-versus-topic. The comment at `delivery.ts:73` reasons carefully about topic-before-opportunity and is right about those two — **it does not notice that its own transaction is already holding the article row.**
**It is a regression from `R-OVERRIDE-TOPIC` (`77aed3c`, merged tonight).** That card added the topic write inside the delivery transaction, which is what created the article→topic edge, and wrote the ordering comment while fixing the deadlock it *was* looking for. The fix was right about the pair it examined and the new edge was not examined.
**What a merchant gets today:** the losing transaction throws. `cancelDay` catches only its own refusal type, so a deadlock propagates out of the dismissal route as a server error rather than the clean "this has already moved on" answer the whole path is built to give. The delivery side is inside a worker step and would be retried.
**How it presents, and this is why the gate nearly missed it:** `packages/jobs/src/publish/complete-opportunity.test.ts` — "settles a publication racing a dismissal one way or the other, never half of each" — fails **intermittently**, because a lock-order bug needs the unlucky interleaving. It failed twice in about five runs and then passed three in a row. **It is NOT one of `R-TESTDB`'s wall-clock tests and must not be added to that list**: those go red under load and pass in isolation; this one failed *in isolation*, with a Postgres deadlock and a stack trace, at a moment the machine was busy for other reasons. Treat a red here as real.
Read first: `DECISIONS.md` 2026-09-07 `R-OVERRIDE-TOPIC` entries (the deadlock that card *did* find, and its reasoning); main §8.7, §9.4; invariants 15 (guarded transitions) and 18 (all account work serialises under `pg_advisory_xact_lock(account_id)`).
**The design question the card must answer rather than patch around:** invariant 18 says all work for one account serialises under an advisory lock. If both of these paths took that lock, no ordering would matter. **Find out why they do not** — one is a worker step and one is a request — and say so. Reordering the locks is the small fix and may be right; the advisory lock is the fix that stops the next inversion too. Recommend one, in the report, with what it costs.
Done when: a publish racing a dismissal always settles one way or the other and **never raises a deadlock**, proved by a test that drives the race hard enough to have caught this one — the existing test found it twice in five runs, which is not good enough to protect the fix; and whatever ordering rule the answer relies on is stated where the next person to reorder these writes will read it.
...
**DECIDED 2026-09-07: the suggestion goes when the page goes** (`DECISIONS.md`). It cannot be acted on, and leaving it up asks the merchant to keep deciding about something that does not exist.
**Two things the answer did not state, taken by the integrator and reversible in a line each** — say in the report if the build makes either look wrong: the row **moves to a closed status rather than being deleted**, because invariant 10 says expiry never deletes and this row is the only record that we ever suggested it; and **a page the nightly walk finds again brings its suggestion back**, matching what `R-PAGE-GONE-OPTIMIZE` already decided and tested for the button, where a restored page is eligible again with nothing else run.
Read first: `DECISIONS.md` 2026-09-07 `R-PAGE-GONE-OPTIMIZE` entries; main §7.9; invariant 10.
Done when: a suggestion whose page is gone stops being offered as work; the row is closed by a guarded status change and never deleted; a page the walk restores brings its suggestion back; and re-detection does not produce a second row for the same page, which the dedupe index already forbids.
...
**A second, related question the same card raised — ANSWERED by the founder on 2026-09-07 and built:** the calendar draws a day the merchant published over our objection as **published, with no refusal card**; the article page keeps both facts, that we objected and that they went ahead. The two surfaces answer different questions and were deliberately allowed to differ. Journalled and pinned by a test; the neighbouring case, an overridden article that has not yet gone out, still names the reason we held it.
...
**R-HANDLE-RENAME — a merchant renaming our article's address breaks the link for ever** · **DECIDED 2026-09-07 — Lane C**
...
**The founder chose the notification, not a second recognition key** (`DECISIONS.md` 2026-09-07). Matching on title or on the marker metafield was declined: it costs a lookup and can be wrong, and being wrong here means claiming a merchant's own writing as ours.
**And it is far cheaper than the question implied — verified after the decision.** `articles/update` is **already** a topic the app subscribes to (`packages/core/src/catalog/webhooks.ts:24`), already becomes an `article_updated` event, and that event **already** makes the walk re-read that one article (`packages/core/src/inventory/events.ts:68`). So there is no new subscription, no new scope, no new receiver and no second failure mode. The work is: when the walk re-reads an article whose Shopify id is one we published, follow the address rather than leaving the stale row `live`. **All of it is Lane C's ground.**
**What this answer does not cover, and it is the larger half of the stores:** an export-mode account, where the merchant pastes our article into their own blog, never gives us a Shopify article at all — no notification, no id. Recognition there stays by address and a rename still detaches. **Export is the default delivery mode**, so state plainly in the report that this closes the auto-publish half only, and do not let a test imply otherwise.
...
Done when: renaming a published article's handle on a store we publish to does not detach it from the article that produced it — proved by feeding the receiver a rename and asserting our row carries the new address; the stale row is never left `live` at an address the store does not serve; and the export-mode limit is stated in the report rather than papered over.
...
**R-REWRITE-PLACE — a rewrite would publish a second article competing with the first** · **DECIDED 2026-09-07 — Lane D**
...
**ANSWERED by the founder directly on 2026-09-07 and journalled: a rewrite replaces the article in place — same address, same post, updated words.** This matches the answer relayed the same day through the `sortiva-a8` session, so both paths agree. The accepted cost is that a rewrite overwrites words a merchant may have edited themselves.
Carry this into the build: `R-PUBLISH-2` decided that a repair sends only title, body and summary and leaves the merchant's address, tags and unpublish choice alone. **The same restraint applies to a rewrite and is not automatic** — an in-place update that rewrote more than those three fields would take back edits the merchant made, which is the cost the founder accepted, not an invitation to widen it.
Done when: a rewrite updates the existing post through the conditional update rather than creating a second one, and **cannot fall back to create** — proved by a test that removes the remote id and asserts the rewrite fails rather than posts; the update carries title, body and summary only, so the merchant's address, their own tags and their choice to unpublish survive it; and something downstream can tell a rewrite from new coverage, which nothing can today.
...
**The decision:** recording a publish attempt means storing something we do not store today. The lane's recommendation is a small append-only table, one row per attempt and how it ended. The cheaper-looking alternative — a fourth state on the claim row — needs surgery on the unique index that is the only thing standing between a crash and a merchant getting the same article posted to their shop twice, which is not a trade worth making for a counter.
Done when: a refused publish leaves a record, the brake reads it, and the stand-in leaves `pnpm stubs:report` and its record in `seams-wired.test.ts` is deleted.
...
**R-DISMISS-DOES-NOTHING — "not interested" is not recorded anywhere anything reads, so declined advice comes back every Monday** · **DONE, merged as `259fd7b`** · **Lane C** · **live on merchant screens now** · found 2026-09-08 by the lane sent to do `R-DISMISS-FOLLOWS-RENAME`, and verified independently by the integrator
Scope: a merchant presses "not interested" on a suggestion. It leaves the screen. **The next weekly scan puts it straight back** — no rename required, nothing unusual, every week, for as long as they keep saying no.

**What is actually there.** A table exists for exactly this — `dismissed_opportunities`, one row per (store, kind of suggestion, page address) — with a writer, a reader and an undo. Its own schema comment states the guarantee: "so a dismissed signal is never re-proposed." **The writer and the reader have no callers outside their own tests.** The button the merchant presses reaches a differently-named function that flips the row's status and writes no marker; the weekly scan never asks. Verified by resolving callers of all four functions: `isDismissed` is called only by `packages/db/src/opportunities.test.ts`.

**And the status alone cannot carry it.** The unique index that stops a re-detected suggestion duplicating covers only *open* statuses (`packages/db/src/schema/opportunities.ts`), deliberately, so history stays queryable. A dismissed row sits outside it, so the scan inserts a fresh row rather than recognising the old one. Both halves have to be true for the defect; both are.

**The one dismissal that does work is a different thing.** A dismissal that also cancels a booked calendar day writes a fingerprint of the topic's *search term*, read only by the calendar. A suggestion to improve an existing store page has no calendar day, so dismissing one writes nothing at all.

**Design note, not a decision.** Making the marker and the status flip happen in the same transaction — inside `dismissOpportunityGuarded` — keeps them from ever diverging, and keeps the whole card inside Lane C (the opportunities repository, the scan, and `apps/web/app/api/opportunities` are all Lane C's). The alternative, writing the marker from the route handler, has two writers for one fact.
Read first: `packages/db/src/repositories/opportunities.ts` (all four dismissal functions and `dismissOpportunityGuarded`); `packages/db/src/schema/opportunities.ts` (the partial index and the table comment); `packages/jobs/src/generation/veto-topic.ts`; the scan's detection write; main §7.9; invariant 10.
Done when: a dismissed suggestion is not re-proposed by the next scan, proved by a test that dismisses one and then runs a detection that would otherwise recreate it; the "show dismissed" undo still works; and the schema comment claiming the guarantee is either true or gone.
**Blocks `R-DISMISS-FOLLOWS-RENAME`** (the founder's answer (a) of 2026-09-08), which is small once this exists — the nightly walk already carries open suggestions to a renamed post's new address inside the rename's own transaction, and the dismissal joins them there.

**R-DISMISS-DRIFT — the same defect on a second path: the drift sweep re-proposes refused advice** · Lane D, small · found 2026-09-08 by the lane that fixed the first path
Scope: `packages/jobs/src/drift/sweep.ts` writes opportunity rows — broken product references and similar — and does not consult the not-interested list. So a merchant who refuses one of those gets it back, exactly as they did from the weekly scan before tonight. The fix is the same shape and the helper already exists; it is a different lane's file, which is the only reason it was not done at the same time.
**ANSWERED 2026-09-08, and it splits the card in two.** `packages/jobs/src/generation/request-refresh.ts` (the merchant's own "refresh this article" press) also does not consult the list, and the founder decided it **should not**: "they asked to refresh on purpose." An earlier "not interested" is us inferring what a merchant wants; the button is them telling us, and the later instruction wins. So the drift sweep is a defect to fix and the refresh path is behaviour to make deliberate — a comment saying so, and a test asserting a dismissed article can still be refreshed on request, so nobody later tidies up "the one path that forgot to check" and breaks it.
Read first: `DECISIONS.md` 2026-09-08 `R-DISMISS-DOES-NOTHING`; the helper it added in `packages/db/src/repositories/opportunities.ts`; invariant 10.
Done when: a refusal survives the drift sweep, proved the same way — dismiss one, run a pass that would otherwise recreate it; and the refresh request's behaviour is decided rather than incidental.

**R-DISMISS-DUPLICATE-NAME — two different functions called `dismissOpportunity`, and the collision is how the last defect hid** · Lane D, small
...
- **A source scan does not work, and the numbers say so rather than my judgement.** Checking whether a registering file mentions the lock classifies only 5 of 33 as taking it — far too strict, because most registrations delegate. Following imports up to four hops instead classifies **29 of 33** as able to reach it, including the spend-cap sweep, which does no account work at all. A check that says yes to 29 of 33 is not a check; shipping it would have been another guarantee nobody actually gets.
- **Option A — the honest cheap one.** Two declared lists: account-serialised, and not-account-work with a reason each, mirroring `UNGATED_TASKS`. A test asserts every registered task is in exactly one and that no entry names a task that no longer exists. **This does not prove the lock is taken.** What it does is make a new task fail until a person classifies it, which is precisely the failure mode here — nobody looked. Cost: about thirty judgements, each needing the task read.
- **Option B — the structural one.** `registerTask` takes an explicit `accountWork` declaration, and the wrapper it already applies checks, at run time, that a handler declared as account work actually entered the lock — the lock setting a flag in async-local storage for the wrapper to read afterwards. This genuinely proves it, in tests and in production, and it is the same automatic-wrapping argument the registry already makes for kill switches: "a switch that half the code paths consult is not a switch". **Cost: a change at all 33 registration sites, across six lanes' directories**, which is why the integrator did not take it unilaterally.
Recommendation: **B**, because A is a reminder wearing a check's clothes and this file is where the difference matters. But B is a cross-lane change and wants deciding rather than assuming.
...
**A third test, and a genuinely different cause — added 15:15.** `packages/providers/src/shopify/limiter.test.ts`, *"paces real requests and carries the cursor from one page to the next"*, fails with `expected 75 to be greater than or equal to 90`: it measures the real gap the rate limiter leaves between two requests to a real local server. It failed **twice consecutively** at load average 82, and passed **three times out of three** run alone. So unlike the two above, this one is straightforwardly load-sensitive — and note the direction, which is the interesting part: under contention the measured gap came out **shorter** than the limiter was asked to leave, not longer.
**Whose fault the load is: mine.** The integrator runs the full suite as a gate while lane sessions are working, and that combination is what puts the machine at 80-plus. Lanes were forbidden from running the full suite for exactly this reason and the integrator was not.
**Then a clean experiment, 15:19, which settles the mechanism for the pacing test.** Run alone it passes, even at load average 64. Run inside the full suite it fails. Run inside the full suite **with the new `screen-reads.test.ts` excluded, it passes** — and the network test fails instead, as it does regardless. So it is not machine load: it is **how many test files the runner has in flight at once**, and `R-SCREEN-READS` (merged 15:12, 1,138 lines driving twenty-five real components with real network calls) is heavy enough to push it over. It passed in every full run before that merge.
That makes it a live risk to any continuous-integration run, not a local curiosity, and it is a **test** problem rather than a product one: the rate limiter itself is fine.
Whoever takes it should decide between making the assertion independent of real scheduling — while keeping what the test is for, which is that the limiter really paces requests to a real server — and telling the runner to give that file a lane of its own. The first is better if it can be done without hollowing the test out.

So there are two problems here wearing one costume, and they want separating: **tests that reach the real network** (the two above, failing at exactly the runner's default timeout), and **tests that assert on real elapsed time** (this one, failing on a measurement). A fix for one is unlikely to be a fix for the other.
Note: capture the runner's output to a file rather than piping it through `grep` — that is why two of the first three failures could not be named.
