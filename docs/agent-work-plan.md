...
Also here, and it is a **product question rather than a defect**: `packages/jobs/src/generation/request-refresh.ts` (the merchant's own "refresh this article" press) does not consult the list either. Arguably right — the merchant is asking for it — but nobody has decided, and the schema comment now names it as an unconsulted path.
