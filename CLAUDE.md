...
**Effectively-once**
18. The queue is at-least-once; every worker is effectively-once: idempotency keys are derived from inputs (never random), completed keys return stored output, all account work serialises under a per-account advisory lock, any step > 60 s checkpoints. The lock is **session-scoped, on its own dedicated connection** — deliberately *not* `pg_advisory_xact_lock`, which cannot coexist with the checkpoint rule in this same sentence: a transaction-scoped lock is held until its transaction commits, so anything written inside it is invisible until the step ends, and a checkpoint nobody can read until the end is not a checkpoint. That substitution costs three properties the transaction-scoped form gives free, and `packages/jobs/src/runtime/lock.ts` buys each back explicitly: every failure path unlocks or destroys the connection rather than returning it to the pool still holding the lock; a bounded `lock_timeout` turns a stuck holder into a loud retryable failure; and re-entry throws instead of deadlocking. (main §14.3.1–14.3.4; `DECISIONS.md` 2026-09-08)
