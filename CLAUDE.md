...
- Thresholds: `packages/rules` only, each with its spec § in a comment. Copy: `packages/ui/strings/*.json` only (externalised from day one), canonical strings keyed by their Appendix A row.
...
- Commit message: `T<M>.<n>: <title> (main §…, tech §…, ui §…)`. One PR per card, from the lane's worktree branch.
